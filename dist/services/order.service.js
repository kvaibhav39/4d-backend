...
class OrderService {
    ...
    async handleBookingCancellation(bookingId, orgId, refundAmount) {
            ...
            for (let i = 0; i < activeBookings.length; i++) {
                ...
                if (bookingRemaining > 0 && remainingToDistribute > 0) {
                    ...
                    booking.payments.push({
                        ...
                        amount: distributedAmount,
                        at: new Date(),
                        note: `₹${distributedAmount.toFixed(2)} transferred to ${productTitle}`,
                    });
        ...
        if (finalRefundAmount > maxRefund) {
            throw new Error(`Refund amount (₹${finalRefundAmount.toFixed(2)}) cannot exceed maximum refund (₹${maxRefund.toFixed(2)}) after transfers.`);
        }
    ...
    async cancelOrder(orderId, orgId, refundAmount, refundNote) {
        ...
        if (actualRefundAmount > totalPaid) {
            throw new Error(`Refund amount (₹${actualRefundAmount.toFixed(2)}) cannot exceed total paid (₹${totalPaid.toFixed(2)})`);
        }
                ...
                if (bookingPaid > 0) {
                    ...
                    if (distributedRefund > 0) {
                        ...
                        booking.payments.push({
                            ...
                            at: new Date(),
                            note: refundNote ||
                                `Order cancellation refund (₹${distributedRefund.toFixed(2)} of ₹${actualRefundAmount.toFixed(2)})`,
                        });
                        // Update booking amounts
                        const totalPaidAfterRefund = booking.payments
                            .filter((p) => p.type === "ADVANCE" || p.type === "PAYMENT_RECEIVED")
