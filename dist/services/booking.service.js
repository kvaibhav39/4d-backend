...
class BookingService {
    ...
    async createBooking(data) {
        ...
        const { orgId, orderId, productId, categoryId, fromDateTime, toDateTime, decidedRent, advanceAmount, additionalItemsDescription, overrideConflicts, } = data;
    ...
    async issueProduct(id, orgId, paymentAmount, paymentNote) {
        ...
        if (paymentAmount !== undefined && paymentAmount > 0) {
            ...
            if (allowedPaymentAmount < paymentAmount) {
                throw new Error(`Payment amount (₹${paymentAmount.toFixed(2)}) exceeds remaining amount (₹${currentRemaining.toFixed(2)}). Maximum allowed: ₹${currentRemaining.toFixed(2)}.`);
            }
            // Add payment entry as PAYMENT_RECEIVED (normal payment on issue)
            const defaultNote = `Payment received ₹${allowedPaymentAmount.toFixed(2)}`;
    ...
    async addPayment(id, orgId, paymentData) {
        ...
        if (paymentData.type === "REFUND") {
            ...
            if (paymentData.amount > currentTotalPaid) {
                throw new Error(`Refund amount (₹${paymentData.amount.toFixed(2)}) cannot exceed total paid amount (₹${currentTotalPaid.toFixed(2)}). Maximum refund allowed: ₹${currentTotalPaid.toFixed(2)}.`);
            }
