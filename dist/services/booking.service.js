...
class BookingService {
  ...
  async listBookings(filters) {
    ...
    if (search && search.trim()) {
      ...
      const { Order } = await Promise.resolve().then(() =>
        __importStar(require("../models/Order"))
      );
      const { Product } = await Promise.resolve().then(() =>
        __importStar(require("../models/Product"))
      );
  ...
  async updateBooking(id, orgId, data) {
    ...
    if (data.productId !== undefined) {
      ...
      existing.productId = new mongoose_1.default.Types.ObjectId(
        data.productId
      );
    }
    ...
    if (data.categoryId !== undefined) {
      if (data.categoryId) {
        existing.categoryId = new mongoose_1.default.Types.ObjectId(
          data.categoryId
        );
      } else {
    ...
    if (updated.orderId) {
      const { OrderService } = await Promise.resolve().then(() =>
        __importStar(require("./order.service"))
      );
  ...
  async addPayment(id, orgId, paymentData) {
    ...
    const paymentType =
      booking.status === "BOOKED" ? "ADVANCE" : "PAYMENT_RECEIVED";
