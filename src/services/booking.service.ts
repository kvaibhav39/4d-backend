...
export class BookingService {
  ...
  async listBookings(filters: ListBookingsFilters): Promise<PaginatedResponse<any>> {
    ...
    if (search && search.trim()) {
      ...
      if (total === 0) {
        return {
          data: [],
          pagination: PaginationHelper.getMeta(page, limit, 0),
        };
      }
   ...
   */
  private calculateTotalAdvance(payments: any[]): number {
   ...
   */
  private async updateBookingAmounts(booking: any): Promise<any> {
